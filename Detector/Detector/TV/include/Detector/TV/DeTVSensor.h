...
namespace LHCb::Detector {
  ...
  namespace detail {
     ...
     *  \version  1.0
     */
    struct DeTVSensorObject : DeIOVObject {
      /// Cache of local x-cooordinates
      std::array<double, TV::NSensorColumns> local_x;
      /// Cache of x-pitch
      std::array<double, TV::NSensorColumns> x_pitch;

      /// Dimensions of the sensor active area
      double sizeX     = 0e0;
      double sizeY     = 0e0;
      double thickness = detail::toLHCbLengthUnits( dd4hep::_toDouble( "TV:Thickness" ) );
      /// Length of chip active area
      double chipSize = detail::toLHCbLengthUnits( dd4hep::_toDouble( "TV:ChipSize" ) );
      /// Distance between two chips
      double interChipDist = detail::toLHCbLengthUnits( dd4hep::_toDouble( "TV:InterChipDist" ) );
      /// Cell size of pixels
      double pixelSize = detail::toLHCbLengthUnits( dd4hep::_toDouble( "TV:PixelSize" ) );
      /// Cell size in column direction of elongated pixels
      double interChipPixelSize = detail::toLHCbLengthUnits( dd4hep::_toDouble( "TV:InterChipPixelSize" ) );
      /// Global Z position
      double zpos = 0e0;
      ...
      std::set<TVChannelID> badChannels = {};

      /// Number of chips per ladder
      unsigned int nChips{(unsigned int)dd4hep::_toInt( "TV:NChips" )};
      /// Number of columns and rows
      unsigned int nCols{(unsigned int)dd4hep::_toInt( "TV:NColumns" )};
      unsigned int nRows{(unsigned int)dd4hep::_toInt( "TV:NRows" )};
      /// Sensor ans module number
      TVChannelID::SensorID sensorNumber{0};
      ...
      /// constructor
      DeTVSensorObject( const dd4hep::DetElement& de, dd4hep::cond::ConditionUpdateContext& ctxt, unsigned int moduleId,
                        bool _isLeft );
   ...
   *  \version  1.0
   */
  struct DeTVSensor : DeIOVElement<detail::DeTVSensorObject> {
    ...
    TVChannelID::SensorID sensorNumber() const { return this->access()->sensorNumber; }
    ...
    bool OKPixel( TVChannelID channel ) const {
    ...
    /// Cache of local x-cooordinates
    const std::array<double, TV::NSensorColumns>& xLocal() const { return this->access()->local_x; };
    /// Cache of x-pitch
    const std::array<double, TV::NSensorColumns>& xPitch() const { return this->access()->x_pitch; };
    ...
    ROOT::Math::XYZPoint channelToLocalPoint( const TVChannelID channel ) const {
    ...
    ROOT::Math::XYZPoint channelToGlobalPoint( const TVChannelID channel ) const {
    ...
    static void getGlobalMatrixDecomposition( const detail::DeTVSensorObject& sensor, ROOT::Math::Rotation3D& ltg_rot,
    ...
    bool isLong( const TVChannelID channel ) const {
    ...
    bool pointToChannel( ROOT::Math::XYZPoint const& point, bool local, TVChannelID& channel ) const;
    /// Calculate the position of a given pixel.
    ROOT::Math::XYZPoint channelToPoint( const TVChannelID& channel, const bool local ) const {
      const double               x = xLocal()[channel.scol()];
      const double               y = ( to_unsigned( channel.row() ) + 0.5 ) * pixelSize();
...
inline bool LHCb::Detector::DeTVSensor::pointToChannel( ROOT::Math::XYZPoint const& point, bool local,
                                                        TVChannelID& channel ) const {
  ...
  for ( unsigned int i = 0; i < numChips(); ++i, x0 += dx ) {
    ...
    if ( x < chipSize() + 0.5 * interChipDist() ) {
      // Set the chip number.
      channel.setChip( TVChannelID::ChipID{i} );
      ...
      channel.setCol( TVChannelID::ColumnID{col} );
      channel.setRow( TVChannelID::RowID{row} );
      break;
    }
  }
  return true;
