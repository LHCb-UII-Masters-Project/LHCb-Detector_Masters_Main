...
LHCb::Detector::detail::DeTVSensorObject::DeTVSensorObject( const dd4hep::DetElement&             de,
    ...
    , sensorNumber( TVChannelID::SensorID{de.id() + TV::NSensorsPerModule * moduleId} )
  ...
  if ( condcs.isValid() ) {
    ...
    if ( node.size() != 3 ) {
      dd4hep::printout( dd4hep::WARNING, "DeTVSensorObject",
  ...
  for ( unsigned int col = 0; col < TV::NSensorColumns; ++col ) {
    // Calculate the x-coordinate of the pixel centre and the pitch.
    double x0    = ( col / TV::NColumns ) * ( chipSize + interChipDist );
    double x     = x0 + ( col % TV::NColumns + 0.5 ) * pixelSize;
...
void LHCb::Detector::detail::DeTVSensorObject::print( int indent, int flg ) const {
  std::string prefix = getIndentation( indent );
  DeIOVObject::print( indent, flg );
  if ( flg & SPECIFIC ) {
    dd4hep::printout( dd4hep::INFO, "DeTVSensor", "%s+  >> Module:%d Sensor:%d %s %d Chips Rows:%d Cols:%d",
                      prefix.c_str(), moduleNumber, sensorNumber, isLeft ? "Left" : "Right", nChips, nCols, nRows );
  }
  if ( flg & DETAIL ) {
    dd4hep::printout( dd4hep::INFO, "DeTVSensor", "%s+  >> Thickness:%g ChipSize:%g Dist:%g Pix-Size:%g Dist:%g",
                      prefix.c_str(), thickness, chipSize, interChipDist, pixelSize, interChipPixelSize );
    dd4hep::printout( dd4hep::INFO, "DeTVSensor", "%s+  >> SizeX: %g SizeY: %g local:%ld pitch:%ld", prefix.c_str(),
                      sizeX, sizeY, local_x.size(), x_pitch.size() );
  }
  dd4hep::printout( dd4hep::INFO, "DeTVSensor", "%s+  >> Info: %s Noise:%s Readout:%s", prefix.c_str(),
