...
static long test_dump_TV( dd4hep::Detector& description, int argc, char** argv ) {
  bool        help = false;
  ...
  dd4hep::DetElement de       = description.detector( "TV" );
  ...
  LHCb::Detector::DeTV TV = slice->get( de, LHCb::Detector::Keys::deKey );
  {
    for ( const auto& s : TV->sides ) {
      for ( const auto& m : s.modules ) {
        LHCb::Detector::DeTVModule module( &m );
        ...
        dd4hep::printout( dd4hep::INFO, "Module", "TV Module: %2d Sensors: %s Position: %8.3f %8.3f %8.3f [mm]  %s",
    ...
    for ( const auto& s : TV->sides ) {
      for ( const auto& m : s.modules ) {
        ...
        for ( const auto& l : module->ladders ) {
          LHCb::Detector::DeTVSensor sens( &( l.sensor ) );
        ...
        dd4hep::printout( dd4hep::INFO, "Module",
                          ...
                          global.Z() / dd4hep::mm, dd4hep::typeName( typeid( module_held_object ) ).c_str() );
        for ( const auto& ladder : module->ladders ) {
          LHCb::Detector::DeTVSensor sens( &( ladder.sensor ) );
...
DECLARE_APPLY( LHCb_TEST_dump_TV, test_dump_TV )
