...
namespace {
  ...
  TVBuild::TVBuild( dd4hep::Detector& dsc, xml_elt_t e, dd4hep::SensitiveDetector sens )
    ...
    sensorSilicon = description.material( "TV:SensorSilicon" );
  }
  ...
  void TVBuild::build_module() {
    ...
    // //New parameters, not yet needed
    // double ClosestPixel = dd4hep::_toDouble( "TV:ClosestPixel" );
    // double DefaultClosestPixel = dd4hep::_toDouble( "TV:DefaultClosestPixel" );
    // double FurthestPixel = dd4hep::_toDouble( "TV:FurthestPixel" );
    ...
    double ChipActiveSizeX = dd4hep::_toDouble( "TV:ChipActiveSizeX" );
    double ChipActiveSizeY = dd4hep::_toDouble( "TV:ChipActiveSizeY" );
    ...
    dd4hep::Material chipSilicon = description.material( "TV:ChipSilicon" );

    // Description of single Chip
    dd4hep::Box    boxChip( ChipTotalSizeX / 2.0, ChipTotalSizeY / 2.0, ChipThick / 2.0 );
    dd4hep::Volume lvChip( "lvChip", boxChip, chipSilicon );
    ...
    dd4hep::Box    boxBumpBonds( ChipActiveSizeX / 2.0, ChipActiveSizeY / 2.0, BumpBondsThick / 2.0 );
    ...
    int    NChips    = dd4hep::_toInt( "TV:NChips" );
    double pos_X_add = ChipActiveSizeX + Chip2ChipDist;               // amount to add between diff chips
    double pos_Y     = SiSizeY + ChipTopEdgeY - ChipTotalSizeY / 2.0; // this doesn't change
    double pos_X;
    // want to go from left to right
    if ( NChips % 2 ) {
      pos_X = SiSizeX / 2.0 - ( NChips - 1 ) / 2 * ChipActiveSizeX - ( NChips - 1 ) / 2 * Chip2ChipDist;
    } else {
      pos_X = SiSizeX / 2.0 - ( NChips / 2 - 0.5 ) * ChipActiveSizeX - ( NChips / 2 - 0.5 ) * Chip2ChipDist;
    }
    std::string nameOfElement;
    std::string chipNumStr;
    for ( int chipNum = 0; chipNum < NChips; chipNum++ ) {
      if ( chipNum != 0 ) { pos_X += pos_X_add; }
      pv = lvChips.placeVolume( lvChip, dd4hep::Position( pos_X, pos_Y, ChipThick / 2.0 ) );

      chipNumStr = std::to_string( chipNum );

      nameOfElement = "pvChip";
      nameOfElement += chipNumStr;
      pv->SetName( nameOfElement.c_str() );
      pv = lvChips.placeVolume( lvGlue, dd4hep::Position( pos_X, pos_Y, -GlueThick / 2.0 ) );

      nameOfElement = "pvGlue";
      nameOfElement += chipNumStr;
      pv->SetName( nameOfElement.c_str() );

      nameOfElement = "pvBumpBonds";
      nameOfElement += chipNumStr;
      pv = lvChips.placeVolume( lvBumpBonds,
                                dd4hep::Position( pos_X, SiSizeY / 2.0, ChipThick + BumpBondsThick / 2.0 ) );
      pv->SetName( nameOfElement.c_str() );
    }

    // Silicon sensor reduced down to actual size
    dd4hep::Box    detBox( ( SiSizeX ) / 2.0, ( SiSizeY ) / 2.0, SiThick / 2.0 );
    dd4hep::Volume lvDet( "lvDet", detBox, sensorSilicon );
    lvDet.setVisAttributes( description, "TV:Sensor" );
    lvDet.setSensitiveDetector( sensitive );

    // Now add the guard ring
    dd4hep::Material GRSilicon = description.material( "TV:GuardRingSilicon" );

    dd4hep::Box  GRBox( ( SiSizeX + 2 * SiEdge ) / 2.0, ( SiSizeY + 2 * SiEdge ) / 2.0, SiThick / 2.0 );
    dd4hep::Box  GRHole( ( SiSizeX ) / 2.0, ( SiSizeY ) / 2.0, 2.0 * SiThick );
    dd4hep::Tube GRCutTopRight( SiEdge + TV_eps, 2.0 * SiEdge, 2.0 * SiThick, 0.0 * dd4hep::degree,
                                90.0 * dd4hep::degree );
    dd4hep::Tube GRCutTopLeft( SiEdge + TV_eps, 2.0 * SiEdge, 2.0 * SiThick, 90.0 * dd4hep::degree,
                               180.0 * dd4hep::degree );
    dd4hep::Tube GRCutBottomLeft( SiEdge + TV_eps, 2.0 * SiEdge, 2.0 * SiThick, 180.0 * dd4hep::degree,
                                  270.0 * dd4hep::degree );
    dd4hep::Tube GRCutBottomRight( SiEdge + TV_eps, 2.0 * SiEdge, 2.0 * SiThick, 270.0 * dd4hep::degree,
                                   360.0 * dd4hep::degree );

    // Now subtract it
    dd4hep::SubtractionSolid GRSolid( GRBox, GRHole, dd4hep::Position( 0, 0, 0 ) );
    GRSolid = dd4hep::SubtractionSolid( GRSolid, GRCutTopRight, dd4hep::Position( SiSizeX / 2.0, SiSizeY / 2.0, 0 ) );
    GRSolid = dd4hep::SubtractionSolid( GRSolid, GRCutTopLeft, dd4hep::Position( -SiSizeX / 2.0, SiSizeY / 2.0, 0 ) );
    GRSolid =
        dd4hep::SubtractionSolid( GRSolid, GRCutBottomLeft, dd4hep::Position( -SiSizeX / 2.0, -SiSizeY / 2.0, 0 ) );
    GRSolid =
        dd4hep::SubtractionSolid( GRSolid, GRCutBottomRight, dd4hep::Position( SiSizeX / 2.0, -SiSizeY / 2.0, 0 ) );
    dd4hep::Volume lvGuardRingVol( "lvGuardRingVol", GRSolid, GRSilicon );
    lvGuardRingVol.setVisAttributes( description, "TV:GuardRing" );

    dd4hep::Assembly lvSensor( "lvSensor" );
    pv = lvSensor.placeVolume( lvDet, dd4hep::Position( SiSizeX / 2.0, SiSizeY / 2.0, 0 ) );
    pv->SetName( "pvDet" );

    pv = lvSensor.placeVolume( lvGuardRingVol, dd4hep::Position( SiSizeX / 2.0, SiSizeY / 2.0, 0 ) );
    pv->SetName( "pvGR" );
    ...
    for ( unsigned i = 0; i < 3; ++i ) {
      ...
      for ( unsigned nPipe = 1;; ++nPipe ) {
        ...
        if ( build_pipe_tori and ( nPipe == 4 or nPipe == 8 ) ) {
          double        rTorus = getLength( "TV:DeliveryPipeLength5Y" ) * 0.5;
        ...
        dd4hep::CutTube tubeR( rMinR, rMaxR, tubeLenR * 0.5 - TV_eps, 0., 2 * M_PI, bV.X(), bV.Y(), bV.Z(), fV.X(),
                               fV.Y(), fV.Z() );
  ...
  void TVBuild::build_detector() {
    ...
    if ( build_sides ) {
      ...
      for ( int i = 0; i < NoOfStations; ++i ) { // One or two loops -- what would be more efficient in tracking?
        // if ( i!=0 && i!=(NoOfStations-1) ) {continue;}       //for testing
        double station_z   = dd4hep::_toDouble( dd4hep::_toString( i, "TV:Station%02dZ" ) );
        double station_rot = dd4hep::_toDouble( dd4hep::_toString( i, "TV:Station%02dDeltaRot" ) );
        pv                 = lvRight.placeVolume(
            lvModule, dd4hep::Transform3D( dd4hep::RotationZ( station_rot ) ) *
                          dd4hep::Transform3D( dd4hep::Position( 0, 0, station_z - Right2LeftDist / 2.0 ) ) );
        ...
        pv = lvLeft.placeVolume(
            lvModule, dd4hep::Transform3D( dd4hep::RotationZ( station_rot ) ) *
                          dd4hep::Transform3D( rot_left, dd4hep::Position( 0, 0, station_z + Right2LeftDist / 2.0 ) ) );
      ...
      bool corrugated_foil = dd4hep::_toInt( "TV:FoilCorrugated" );

      if ( build_rf_box && corrugated_foil ) {
      ...
      if ( build_rf_foil ) {
        ...
        dd4hep::Position pos_left(
            corrugated_foil
                ? -1 * ( ( dd4hep::_toDouble( "TV:ClosestPixel" ) - 1.5 ) - dd4hep::_toDouble( "TV:RFFoilRadius" ) )
                : 0,
            corrugated_foil
                ? +0 * ( dd4hep::_toDouble( "TV:ClosestPixel" ) - dd4hep::_toDouble( "TV:DefaultClosestPixel" ) )
                : 0,
            corrugated_foil ? dd4hep::_toDouble( "TV:RFFoilLeftPosZ" ) : 0.0 );

        dd4hep::Position pos_right(
            corrugated_foil
                ? +1 * ( ( dd4hep::_toDouble( "TV:ClosestPixel" ) - 1.5 ) - dd4hep::_toDouble( "TV:RFFoilRadius" ) )
                : 0,
            corrugated_foil
                ? 0 * ( dd4hep::_toDouble( "TV:ClosestPixel" ) - dd4hep::_toDouble( "TV:DefaultClosestPixel" ) )
                : 0,
            corrugated_foil ? dd4hep::_toDouble( "TV:RFFoilRightPosZ" ) : 0.0 );
