...
namespace LHCb::Detector {
  ...
  public:
    /// enum defining the VELO side
    enum struct Side { C, A };
    ...
    constexpr VPChannelID( SensorID sensor, ChipID chip, ColumnID col, RowID row, OrfxID orfx = OrfxID{0},
                           OrfyID orfy = OrfyID{0} )
        : VPChannelID{shift<Mask::orfx>( orfx ) | shift<Mask::orfy>( orfy ) | shift<Mask::sensor>( sensor ) |
                      shift<Mask::chip>( chip ) | shift<Mask::col>( col ) | shift<Mask::row>( row )} {}

    /// Constructor with sensor, scol, row, orfx and orfy
    constexpr VPChannelID( SensorID sensor, ScolID scol, RowID row, OrfxID orfx = OrfxID{0}, OrfyID orfy = OrfyID{0} )
        : VPChannelID{shift<Mask::orfx>( orfx ) | shift<Mask::orfy>( orfy ) | shift<Mask::sensor>( sensor ) |
    ...
    /// Get side (left/right)
    [[nodiscard]] constexpr Side sidepos() const { return Side( module() % 2 ); }
    ...
    VPChannelID& setSensor( SensorID sensor ) { return set<Mask::sensor>( sensor ); }

    /// Retrieve orfy
    [[nodiscard]] constexpr OrfyID orfy() const { return OrfyID{extract<Mask::orfy>( m_channelID )}; }

    /// Update orfy
    VPChannelID& setOrfy( OrfyID orfy ) { return set<Mask::orfy>( orfy ); }

    /// Retrieve orfx
    [[nodiscard]] constexpr OrfxID orfx() const { return OrfxID{extract<Mask::orfx>( m_channelID )}; }

    /// Update orfx
    VPChannelID& setOrfx( OrfxID orfx ) { return set<Mask::orfx>( orfx ); }

    /// Retrieve scol
    [[nodiscard]] constexpr ScolID scol2() const { return ScolID{extract<Mask::scol>( m_channelID )}; }

    /// Update scol
    VPChannelID& setScol( ScolID scol2 ) { return set<Mask::scol>( scol2 ); }

    friend std::ostream& operator<<( std::ostream& str, const VPChannelID& obj );

  private:
    unsigned int m_channelID{std::numeric_limits<unsigned int>::max()}; ///< VP Channel ID
