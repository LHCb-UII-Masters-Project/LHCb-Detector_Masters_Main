...
sed -i "s/LCG_AA_project(DD4hep HEAD)/LCG_AA_project(DD4hep ${latest_tag})/"  lcgcmake/cmake/toolchain/heptools-94.cmake

git clone https://gitlab.cern.ch/sft/lcgjenkins.git
mkdir lcgcmake-build
cd  lcgcmake-build
cmake -DLCG_INSTALL_PREFIX=/cvmfs/sft.cern.ch/lcg/releases -DLCG_VERSION=94 -DLCG_IGNORE='DD4hep' -DCMAKE_CXX_STANDARD=17 -DCMAKE_INSTALL_PREFIX=../lcgcmake-install ../lcgcmake
make -j 32
cd ../lcgcmake-install/
../lcgjenkins/extract_LCG_summary.py . x86_64-centos7-gcc7-opt 94 RELEASE
 ../lcgjenkins/LCGRPM/package/createLCGRPMSpec.py -p x86_64-centos7-gcc7-opt --release=1 --match=".*DD4hep.*" -o dd4hep.spec ./LCG_externals_x86_64-centos7-gcc7-opt.txt
rpmbuild -bb dd4hep.spec
